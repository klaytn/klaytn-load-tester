package scNFTTransfer2StepTcWithCheck
...
const Name = "scNFTTransfer2StepTcWithCheck"
...
func Init(mcAccs []*account.Account, scAccs []*account.Account, mcBridgeAddress, scBridgeAddress, _, _, mcNFTAddress, scNFTAddress common.Address) {
...
func Run() {
	...
	rTx, err := account.RequestNFTTransfer2Step(from, to, erc721Addr, bridgeAddr)
	if err == nil {
	...
	fmt.Printf("Fail to RequestNFTTransfer2Step RTX(%v)\n", rTx.Hash().String())
	goto FINISH
...
CHECK_HTX_RECEIPT:
	for i := 0; i <= 1000; i++ {
		...
		if receipt != nil {
			...
			//fmt.Printf("Success ERC721 ApproveTransferFrom. RTX(%v), HTX(%v), elapsed = %v\n", rTx.Hash().String(), hTxHash.String(), boomer.Now()-start)
			goto FINISH
		}
		time.Sleep(100 * time.Millisecond)
	}
	fmt.Printf("fail to ERC721 ApproveTransferFrom. RTX(%v), HTX(%v), elapsed = %v\n", rTx.Hash().String(), hTxHash.String(), boomer.Now()-start)
...
FINISH:
	...
	if err == nil {
		...
		boomer.Events.Publish("request_success", "http", "ApproveTransferFrom NFT 2-step with check "+comment, elapsed, int64(10))

	} else {
		//boomer.Events.Publish("request_failure", "http", "signedtransfer"+" to "+endPoint, elapsed, err.Error())
		boomer.Events.Publish("request_failure", "http", "ApproveTransferFrom NFT 2-step with check "+comment, elapsed, err.Error())
	}
}
