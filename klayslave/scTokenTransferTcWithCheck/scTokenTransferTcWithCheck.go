package scTokenTransferTcWithCheck
...
const Name = "scTokenTransferTcWithCheck"
...
func Run() {
	...
	rTx, err := account.RequestTokenTransfer(from, to, value, erc20Addr)
	if err == nil {
	...
	fmt.Printf("Fail to RequestTokenTransfer RTX(%v)\n", rTx.Hash().String())
	goto FINISH
...
FINISH:
	...
	if err == nil {
		...
		boomer.Events.Publish("request_success", "http", "Transfer ERC20 with check "+comment, elapsed, int64(10))

	} else {
		//boomer.Events.Publish("request_failure", "http", "signedtransfer"+" to "+endPoint, elapsed, err.Error())
		boomer.Events.Publish("request_failure", "http", "Transfer ERC20 with check "+comment, elapsed, err.Error())
	}
}
